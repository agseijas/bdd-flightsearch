...
public class AirlinesCSVFileReader implements AirlinesReader {
    ...
    public Set<Airline> readAirlinesFromIO(final String... params) {
        ...
        try (final Stream<String> streamAirlines = getFilesLineStreamFrom(airlinesFile)) {
            ...
            throw FSException.withMessage("Unable to load airlines");
        }
    ...
    private URI getUriForFilePathResource(final String filePath) throws URISyntaxException {
        if (getClass().getResource(filePath) == null) {
            throw FSException.withMessage("unable to find file: " + filePath);
        }
        return getClass().getResource(filePath).toURI();
    }

    private Airline createAirlineFromAirlineDataLine_InFile(final String airlineDataLine,
            final Set<String[]> allInfantPricesData) {
        final String[] airlineData = airlineDataLine.split(",");

        final String[] infantPriceData = allInfantPricesData.stream()
                .filter(infantPriceForCompany -> airlineIATACodeIsInfantPriceCompanyIATACode(airlineData,
                        infantPriceForCompany))
                .findFirst().orElse(null);
        if (notFound(infantPriceData)) {
            return new Airline.buildWith().origin(airlineData[0]).destination(airlineData[1]).airline(airlineData[2])
                    .price(airlineData[3]).createAirline();
        }
        return new Airline.buildWith().origin(airlineData[0]).destination(airlineData[1]).airline(airlineData[2])
                .price(airlineData[3]).infantPrice(infantPriceData[2]).company(infantPriceData[1]).createAirline();
    }
